...
KMC(KOOK Market Canvas)는 내부 시스템이 아닌 외부 퍼블릭 웹에서 동작하며, 지정 사용자가 언제 어디서나 모바일/PC로 조회하고 작업할 수 있어야 합니다. 이를 위해 **비용 0원(Free Tier)으로 가동하는 클라우드 퍼블리싱 아키텍처**를 다음과 같이 설계합니다.
...
| **데이터베이스 (PostgreSQL)** | **Supabase** 또는 **Neon.tech** | 지식 그래프, 종목/관계 스냅샷 및 즐겨찾기 저장 | 500MB 무료 용량 제공, RESTful API 자동 맵 제공 |
...
[퍼블릭 웹: KMC 클라우드]
 ...
 - Vercel Next.js Web/PWA (지정 사용자 모바일/PC 브라우저 조회)
```
...
### 📦 13.4. 로컬 검증 및 자동 배포 형상관리 지침
KMC 서비스의 모바일 우선 갱신 경험을 최적화하기 위해 다음 형상관리 및 배포 지침을 상시 준수합니다:
1. **로컬 검증 완료 즉시 커밋**: 로컬 개발 서버(`npm run dev`)에서 코드를 점검하고 `npm run build`가 정상적으로 통과하면, 이를 로컬 파일에만 방치하지 않고 즉시 `main` 브랜치에 커밋/푸시(`git push origin main`)합니다.
2. **자동 Vercel 배포 트리거**: 푸시된 소스코드는 GitHub Actions/Vercel CI 파이프라인을 트리거하여 자동으로 클라우드에 갱신 배포됩니다.
3. **사용자 실시간 보고**: 에이전트는 배포가 완료되는 즉시 사용자(사령관)에게 퍼블릭 Vercel 접속 주소를 회신하여 사외 모바일에서도 즉시 검증할 수 있도록 돕습니다.
...
1. **1차 수동 기획 매핑**: 서비스 착수 및 캔버스 초기 생성 시에는 운영자의 기획과 리서치 데이터를 바탕으로 선도주와 밸류체인 관계를 1차로 수동 매핑합니다.
